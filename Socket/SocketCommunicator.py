...
class SocketCommunicator(QObject):
    ...
    update_bandwidth_signal = pyqtSignal(list)
    ...
    def receive_data(self):
        ...
        while self.conn:
            ...
            # 计算带宽
            bandwidth_mbps = (total_data_received * 8) / (elapsed_time * 1024 * 1024)

            self.bandwidth.append(bandwidth_mbps)
            self.update_bandwidth_signal.emit([total_data_received, elapsed_time, bandwidth_mbps])
            ...
            if data:
                ...
                if isinstance(deserialized_data, str) and deserialized_data.startswith("Text:"): # receive text
                    ...
                    if self.audio_buffer:
                        self.update_text_signal.emit(f"[IMAGE] Received image, audio data {len(self.audio_buffer)} bytes, image data {len(image_data)} bytes.\n")
                    ...
                    if frame is not None:
                        ...
                        scaled_image = qt_image.scaled(600, 460, Qt.AspectRatioMode.KeepAspectRatio)
                        # self.change_pixmap_signal.emit(scaled_image)
                        # self.audio_buffer = None

                        if self.audio_buffer:
                            # start Wa2Lip Thread
                            self.wav2Lip_thread = Wav2LipThread(scaled_image, self.audio_buffer, self.wav2lip_model, self.task_id)
                            self.wav2Lip_thread.log_text_signal.connect(self.update_text_signal.emit)
                            # print("Init wav2lip thread.\n")
                            self.task_id += 1
                            self.wav2Lip_thread.start()
                            # wav2Lip_thread.wait()
                            # print("End wav2lip thread.\n")
                            self.audio_buffer = None
    ...
    def end_system(self):
        try:
            for i in self.bandwidth:
                print(i)
        except Exception as e:
            print(f"Error :{e}")
        finally:
            print("mean BW: ", np.mean(self.bandwidth))
